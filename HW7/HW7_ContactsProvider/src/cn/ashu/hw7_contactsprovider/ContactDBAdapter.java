...
public class ContactDBAdapter {
    ...
    public ContactInfo getEntry(String searchname) throws SQLException {
        //this works too, as raw sql query statement
        //Cursor c = db.rawQuery("SELECT * FROM tbl1 WHERE NAME = '"+searchname.trim()+"'", null);
    	//c.moveToFirst();
        //parameterized sql query statement, 
        //notice the condition clause to search for particular name is passed into query statement
        Cursor cursor = db.query(DATABASE_TABLE,
        		new String[] {KEY_ID, CONTACT_NAME, CONTACT_PHONE, CONTACT_EMAIL, CONTACT_POSTAL},
        		CONTACT_NAME + "=" + "'" + searchname.trim() + "'", null, null, null,null, null);
        ...
        String postaladdr = cursor.getString(cursor.getColumnIndex(CONTACT_POSTAL));
        ContactInfo result = new ContactInfo(theId, name, phone, email, postaladdr);
        return result;
    }
    
    public List<ContactInfo> getAllEntries(){
    	List<ContactInfo> list = new ArrayList<ContactInfo>();
    	Cursor cursor = db.query(DATABASE_TABLE, new String[]{KEY_ID, CONTACT_NAME, CONTACT_PHONE, CONTACT_EMAIL, CONTACT_POSTAL }, null, null, null, null, null);
    	while(cursor.moveToNext()){
    		int theId = cursor.getInt(cursor.getColumnIndex(KEY_ID));
    	    String name = cursor.getString(cursor.getColumnIndex(CONTACT_NAME));
    	    String phone = cursor.getString(cursor.getColumnIndex(CONTACT_PHONE));
    	    String email = cursor.getString(cursor.getColumnIndex(CONTACT_EMAIL));
    	    String postal = cursor.getString(cursor.getColumnIndex(CONTACT_POSTAL));
			ContactInfo info = new ContactInfo(theId, name, phone,email,postal);
			list.add(info);
		}
    	return list;
    }
...
//        int num = db.delete(DATABASE_TABLE, null, null);
    ...
    public int removeEntry(long _rowIndex) {
        int  num = db.delete(DATABASE_TABLE, KEY_ID+"=?", new String[] { _rowIndex+"" });
        return num;
    }
